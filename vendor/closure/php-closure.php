 ...
 *
 * Handles caching and recompilation of sources.  A recompilation 
 * will occur whenever a source file, or the script calling the 
 * compiler, changes.
 *
 * The class will handle the Last-Modified and 304 redirects for 
 * you, but if you should set Cache-Control headers appropriate for 
 * your needs.
 *
 * Example usage:
 *
 ...
 *   ->write();
 * 
 * @see http://code.google.com/closure/compiler/docs/api-ref.html for 
 * more details on the compiler options.
 * 
 * @see http://code.google.com/p/php-closure/ for the original version
 * of this class. This has been slightly modified to work better with
 * the way naked-utils compiles javascript.
 */

if (!class_exists('PhpClosure')) {
class PhpClosure {
  ...
  function log($msg) {
    if( !$this->_logger ) {
      $this->_logger = new KLogger( $this->_log_dir, KLogger::INFO );
    }
  ...
  /**
   * Adds a source file to the list of files to compile.  Files will 
   * be concatenated in the order they are added.
   */
  function add($file) {
    $this->_srcs[] = $file;
    return $this;
  ...
  /**
   * Sets the directory where the compilation results should be 
   * cached, if not set then caching will be disabled and the 
   * compiler will be invoked for every request (NOTE: this will hit 
   * ratelimits pretty fast!)
   */
  ...
  /**
   * Sets whether to use the Closure Library.  i.e. goog.requires 
   * will be resolved and the library code made available.
   */
  ...
  /**
   * Sets the URL prefix to use with the Closure Compiler service's 
   * code_url parameter.
   *
   * By default PHP-Closure posts the scripts to the compiler
   * service, however,this is subject to a 200000-byte size limit 
   * for the whole post request.
   *
   * Using code_url tells the compiler service the URLs of the 
   * scripts to fetch.  The file paths added in add() must therefore 
   ...
   *   ->write();
   *
   * This assumes your PHP script is in a directory /app/ and that 
   * the JS is in /app/js/ and accessible via HTTP.
   */
   ...
   * invoking a recompile, if necessary.
   *
   * This works exactly the same as write() but does not set the headers.
   *
   * @since naked-utils 0.1
   */
  function writeSansHeaders() {
  ...
  function _isRecompileNeeded($cache_file) {
    ...
    if (!file_exists($cache_file)) { 
      ...
      if( $this->_log ) {
        $this->log( "Cache File Doesn't Exist: " . $cache_file );
      }
    ...
    foreach ($this->_srcs as $src) {
      if (filemtime($src) > $cache_mtime) { 
        ...
        if( $this->_log ) {
          $this->log( "$cache_file File Modified Time: $cache_mtime" );
          $this->log( "$src File Modified Time: " . filemtime($src) );
        }
    ...
    if (filemtime($_SERVER["SCRIPT_FILENAME"]) > $cache_mtime) { 
      ...
      if( $this->_log ) {
        $this->log( "$cache_file File Modified Time: $cache_mtime" );
        $this->log( $_SERVER["SCRIPT_FILENAME"] . " File Modified Time: " . filemtime($_SERVER["SCRIPT_FILENAME"] ) );
      }
